...
def getResnet50():
  with gfile.FastGFile("/home/heelim/BatchDVFS/BatchDVFS/FrozenGraphs/frozen_pnasnet_large.pb",'rb') as f:
...
##    dummy_input = np.random.random_sample((batch_size,224,224,3))
  ...
  with tf.Session(graph=g,config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      # 1531 - 1380 = 151 
      listDVFSSteps = [390, 480, 585, 630, 795, 915, 1035, 1155, 1282, 1380]
      minDVFS = 0
      maxDVFS = 9
      initialBS = 5
      BestDVFS = 5
      ...
      while True:
          ...
          if (time.time() - two_second_start) > 3 :
              ...
              # Heelim :  This condition checks if the maximum power usage observed (max(powerReading)) falls within the desired range (80% to 100% of the power cap). 
              # If this is the case, the power usage readings are cleared (powerReading[:] = []) and no changes are made to the DVFS level.
              if max(powerReading) <= powerCapW and max(powerReading) >=  (0.8 * powerCapW):
                  powerReading[:] = []
                  pass
              # Heelim : This condition checks if the maximum power usage is less than 80% of the power cap. 
              # If so, the power readings are cleared and, if the current DVFS level (BestDVFS) is not already at the maximum (maxDVFS), it is increased by one step. 
              # This is an attempt to utilize more power to possibly increase the performance.  
              elif max(powerReading) < (0.8 * powerCapW):
                  powerReading[:] = []
                  if BestDVFS < maxDVFS:
                      BestDVFS = BestDVFS + 1
                  DVFSLevel = listDVFSSteps[BestDVFS]
              # Heelim : This condition checks if the maximum power usage exceeds the power cap. 
              # If so, the power readings are cleared and, if the current DVFS level is not already at the minimum (minDVFS), it is decreased by one step. 
              # This is done to reduce power usage and avoid going over the cap.
          ...
          if (oldDVFS != DVFSLevel):
            ...
            os.system(f"echo {password} | sudo -S nvidia-smi --applications-clocks=877," + str(DVFSLevel))
            oldDVFS = DVFSLevel
...
if "__main__" in __name__:
  ...
  if f.native:
    ...
    # for debug 
    with open(str(f.result_file) + '_nvidiasmi_output.csv', 'w') as test_file:
       test_file.write('This is a test.')
    
    # os.system('nvidia-smi --query-gpu=timestamp,utilization.gpu,utilization.memory,power.draw,clocks.sm --format=csv ,nounits, -l 1 -f ' + str(f.result_file) + '_nvidiasmi_output.csv &')
    # os.system("nvidia-smi --query-gpu=timestamp,utilization.gpu,utilization.memory,power.draw,clocks.sm --format=csv,nounits -l 1 -f " + str(f.result_file) + "_nvidiasmi_output.csv")
    os.system("nvidia-smi --query-gpu=timestamp,utilization.gpu,utilization.memory,power.draw,clocks.sm --format=csv,nounits -l 1 -f " + str(f.result_file) + "_nvidiasmi_output.csv &")
