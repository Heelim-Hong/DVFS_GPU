...
stepIncrease = 4
# Heelim : Sliding window size
# Initializing the list with -1 for N=1000 times is a way to indicate that there are no valid power readings yet. 
# The list will be updated with the actual power readings in the CountdownTask class 
N = 1000
# Heelim : Records the power draw of the GPU during each iteration of the countdown task
powerReading = [-1] * N
# Heelim : Records the power draw of the GPU during the entire execution of the benchmark
...
# Heelim : Reads a jpg image file and returns a tensor
# Heelim : This function is used to create a dataset from a list of filenames
# Heelim : The function resizes the image to 224x224 pixels, 
...
# Heelim : Loads a frozen ResNet50 graph from a file and returns the graph def
def getResnet50():
  with gfile.FastGFile("/home/heelim/BatchDVFS/Clipper/FrozenGraphs/frozen_inception_v1.pb",'rb') as f:
    graph_def = tf.GraphDef()
    graph_def.ParseFromString(f.read())
  return graph_def

# Heelim : Main function that performs the benchmakring.
# It takes the frozen graph def as input, along with the batch size, image folder, latency, power caop, and result file
...
##    dummy_input = np.random.random_sample((batch_size,224,224,3))
  imageCounter = 0
  outlist=[]
  # Heelim : Creating a dataset from the input image folder, maps the '_parse_function' to each image, and batches the images
  with g.as_default():
    ...
    out = tf.import_graph_def(      
      graph_def=gdef, # Heelim : Imports the ResNet50 graph def
  ...
  # Heelim : Runs the session for each batch size, timing the execution time and power draw of the GPU
      ...
      while True:
          if (time.time() - two_second_start) > 3 :
              ...
              if max(powerReading) <= powerCapW and max(powerReading) >= (0.8 * powerCapW):
                  ...
                  powerReading[:] = []
                  if BestBS + stepIncrease >= MAXINDEX:
                      pass
                  else:
          ...
          print("############### Heelim ###############", len(powerReading))
          if len(powerReading) == 0:
              time.sleep(3)
              #maxPower = max(powerReading)
          # Heelim : Records the throughput (images per sec) achieved for each batch size and report the maximum throughput achieved
          resultsFile.write(
